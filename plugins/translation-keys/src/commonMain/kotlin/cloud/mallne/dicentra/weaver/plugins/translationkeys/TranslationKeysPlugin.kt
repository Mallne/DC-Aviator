...
object TranslationKeysPlugin : AviatorPlugin<TranslationKeysPluginConfig> {
            ...
            if (pluginConfig.onPath) {
                before(AviatorExecutionStages.PathMatching) { context ->
                    ...
                    context.requestParams =
                        context.requestParams.map { (key, value) -> (tk[key] ?: key) to value }.toMap()
                }
            }
                    ...
                    context.networkChain.forEach { net ->
                        net.request?.outgoingContent?.let {
                            net.request?.outgoingContent =
                                SubstitutionKeysSerializer.translate(tk, it, context.dataHolder.json)
                        }
                    }
                }
            }
    ...
    private fun <T : AviatorExecutionContext<out @Serializable Any, out @Serializable Any>> extractKeys(context: T): Map<String, String> {
        val tk = context.dataHolder.route.`x-dicentra-aviator-serviceOptions`?.get("translation-keys")
        val map = tk?.let { context.dataHolder.json.decodeFromJsonElement<Map<String, String>>(serializer(), it) }
        return map ?: emptyMap()
    }
}