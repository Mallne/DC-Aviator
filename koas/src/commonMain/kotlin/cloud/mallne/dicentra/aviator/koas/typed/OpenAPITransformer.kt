...
import cloud.mallne.dicentra.aviator.koas.typed.Model.*
...
private class OpenAPITransformer(private val openAPI: OpenAPI) {
        ...
        when (val type = type) {
            is Type.Array ->
                when (val single = type.types.singleOrNull()) {
                    null -> {
                        ...
                        Union(
                            context = context,
                            cases = resolved.map { Union.Case(context, it.toModel(context).value) },
                            default = null,
            ...
            is Type.Basic ->
                when (type) {
                    ...
                    Type.Basic.String ->
                        if (format == "binary") OctetStream(description)
                        else
                            Primitive.String(
    ...
    fun Schema.isOpenEnumeration(): Boolean {
        val anyOf = anyOf
        return anyOf.size == 2 &&
                anyOf.count { it.resolve().value.enum.isNotEmpty() } == 1 &&
    ...
    private fun Schema.toObject(context: NamingContext): Model =
        when {
            ...
            additionalProperties != null ->
                when (val props: AdditionalProperties = additionalProperties) {
                    // TODO: implement Schema validation
                    is AdditionalProperties.PSchema ->
                        FreeFormJson(description, Constraints.Object(this))

                    is Allowed ->
                        if (props.value) FreeFormJson(description, Constraints.Object(this))
                        else
            ...
            else -> FreeFormJson(description, Constraints.Object(this))
        }
    ...
    private fun allOf(schema: Schema, context: NamingContext): Model {
        ...
        return when {
            ...
            (schema.additionalProperties as? Allowed)?.value == true ->
                FreeFormJson(schema.description, Constraints.Object(schema))
     ...
     */
    private val unionSchemaComparator: Comparator<Union.Case> = Comparator { o1, o2 ->
        val m1 = o1.model
        val m2 = o2.model
        val m1Complexity =
            when (m1) {
                is Object -> m1.properties.size
        ...
        val cases =
            caseToContext
                .map { (resolved, caseContext) ->
                    Union.Case(caseContext, resolved.toModel(caseContext).value)
                }
    ...
    fun toUnionCaseContext(context: NamingContext, case: Resolved<Schema>): NamingContext =
        when (case) {
            ...
            is Resolved.Value ->
                when {
                    context is Named && case.value.type == Type.Basic.String && case.value.enum.isNotEmpty() ->
                        NamingContext.Nested(
                            Named(
                                        ...
                                        Route.Body.Multipart.Value(
                                            resolved.value.properties.map { (name, ref) ->
                                                Route.Body.Multipart.FormData(
                                                    name,
                                                    ref.resolve().toModel(ctx(name)).value
                                                )
                                            },
                                            body.description,
                                            mediaType.extensions
            ...
            operation.responses.responses.entries.associate { (code, refOrResponse) ->
                ...
                when {
                    response.content.contains("application/octet-stream") ->
                        Pair(
                            statusCode,
                            Route.ReturnType(OctetStream(response.description), response.extensions)
                        )
                    ...
                    response.content.contains("application/json") -> {
                        ...
                        val route =
                            when (val resolved = mediaType.schema?.resolve()) {
                                ...
                                null ->
                                    Route.ReturnType(
                                        if (Allowed(true).value) FreeFormJson(response.description, null)
                                        else
                                            throw IllegalStateException(
